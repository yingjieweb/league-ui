...
<script>
  ...
  export default {
    ...
    computed: {
      classes() {
        const devices = ['phone', 'iPad', 'narrowPC', 'pc', 'widePc']
        const classes = [
          this.span && `lol-col-default-span-${this.span}`,
          this.offset && `lol-col-default-offset-${this.offset}`
        ]
        devices.forEach((device) => {
          if (this[device]) {
            if (typeof this[device].span === 'number' && this[device].span !== 0) {
              classes.push(`lol-col-${device}-span-${this[device].span}`)
            }
            if (typeof this[device].offset === 'number' && this[device].offset !== 0) {
...
<style lang="scss" scoped>
  .lol-col {
    $default-span-prefix: lol-col-default-span-;
    @for $n from 1 through 24 {
      &.#{$default-span-prefix}#{$n} {
        width: ($n/24)*100%;
      }
    }

    $default-offset-prefix: lol-col-default-offset-;
    @for $n from 1 through 24 {
      &.#{$default-offset-prefix}#{$n} {
    ...
    @media (min-width: 769px) and (max-width: 992px) {  /*narrowPc*/
    ...
    @media (min-width: 993px) and (max-width: 1200px) {  /*pc*/
